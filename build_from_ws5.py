...
new_vars = [
    ...
    },
    {
        ...
        'parameter': [
            ...
            {'type': 'LIST', 'key': 'map', 'list': [
                {'type': 'MAP', 'map': [{'type': 'TEMPLATE', 'key': 'key', 'value': 'strict-opt-in'}, {'type': 'TEMPLATE', 'key': 'value', 'value': 'opt-in'}]},
                {'type': 'MAP', 'map': [{'type': 'TEMPLATE', 'key': 'key', 'value': 'notice-opt-out'}, {'type': 'TEMPLATE', 'key': 'value', 'value': 'opt-out'}]},
                {'type': 'MAP', 'map': [{'type': 'TEMPLATE', 'key': 'key', 'value': 'none'}, {'type': 'TEMPLATE', 'key': 'value', 'value': 'opt-out'}]}
            ]},
    ...
    },
    {
        ...
        'parameter': [
            {'type': 'TEMPLATE', 'key': 'input', 'value': '{{regex - legal region archetype}}'},
            {'type': 'LIST', 'key': 'map', 'list': [
                {'type': 'MAP', 'map': [{'type': 'TEMPLATE', 'key': 'key', 'value': 'strict-opt-in'}, {'type': 'TEMPLATE', 'key': 'value', 'value': 'We require your active consent to use cookies for advertising and analytics.'}]},
                {'type': 'MAP', 'map': [{'type': 'TEMPLATE', 'key': 'key', 'value': 'notice-opt-out'}, {'type': 'TEMPLATE', 'key': 'value', 'value': 'We use cookies for optimization. You have the right to opt-out.'}]}
            ]},
...
# ── Add consent + trigger 32 to existing tracking tags ──
for tag in cv['tag']:
    ttype = tag.get('type', '')
    name = tag.get('name', '')

    if ttype == 'gclidw':
        continue  # skip Conversion Linker

    if ttype in ('awct',) or 'AW' in name or 'ads' in name.lower():
    ...
    tag['consentSettings'] = {
        ...
        'manualConsentConstraint': [{'type': 'TEMPLATE', 'value': consent_val}]
    }
